...
Chlorophyll concentration is a proxy used in water quality monitoring for algal biomass. Large chlorophyll values are indication of algal blooms and important water quality parameter. Chlorophyll concentration in natural waters is driven by a complex interaction of temperatures, nutrients, hydrodynamic conditions that is not well quantified and can be quite stochastic in nature. 
...
model{
  ...
  for(i in 2:n){
      # Process model
      chla_latent[i] <- beta1 * chla_latent[i-1] + beta2 * air_temp[i]
      chla_pred[i] ~ dnorm(chla_latent[i], tau_process)

      # Data model
      y[i]  ~ dnorm(chla_pred[i], tau_obs)
  }
}
"
```
...
pred_latent <- matrix(NA, num_samples, n_days)   
forecast <- matrix(NA, num_samples, n_days) 
forecast_df <- NULL
...
for(i in 1:num_samples){
  ...
  pred_latent[i,1] <- chain$chla_latent[sample_index]
  forecast[i, 1] <- rnorm(1, pred_latent[i,1], sd = data$sd_obs)
  
  #loop over forecast days
  for(j in 2:n_days){
    pred <- pred_latent[i,j-1] * chain$beta1[sample_index] +
      noaa_future_site_ens$air_temperature[j] * chain$beta2[sample_index]
    
    pred_latent[i,j] <- rnorm(1, pred, chain$sd_process[sample_index])
    forecast[i,j] <- rnorm(1, pred_latent[i,j], sd = data$sd_obs)
